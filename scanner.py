...
def update_google_sheet_combined(found_data):
    try:
        # GitHub Secrets에서 키 불러오기
        key_content = os.environ.get('GSPREAD_KEY')
        if not key_content:
            print("❌ GSPREAD_KEY 환경 변수가 없습니다.")
            return
        ...
        # [핵심] 모든 종목의 분석 내용을 하나의 문자열로 합치기
        combined_report = ""
        for item in found_data:
            combined_report += f"[{item['ticker']}] ${item['price']} ({item['readiness']}%)\n{item['analysis']}\n"
            combined_report += "-" * 30 + "\n" # 종목 간 구분선

        # 시트의 새 줄에 [날짜, 종목 수, 통합 리포트] 기록
        now = datetime.now().strftime('%Y-%m-%d %H:%M')
        worksheet.append_row([now, f"{len(found_data)}개 종목", combined_report])
        
        print("✅ 통합 리포트 시트 업데이트 성공!")
    except Exception as e:
        print(f"❌ 시트 업데이트 실패: {e}")

def analyze_with_gemini(ticker, readiness, price, vol_ratio, obv_status):
    try:
        # 지연 방지를 위해 gemini-3-flash-preview 모델 사용
    ...
    try:
        ...
        if readiness >= 90 and vol_p > 1.3:
            ...
            return {
                ...
                'price': round(close.iloc[-1], 2), 'analysis': analysis
            }
    except: return None
